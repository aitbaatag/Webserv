...
void ServerConfigParser::parseListen(std::vector<std::string > &lineTokens, std::size_t &idx)
{
	if (idx + 2 >= lineTokens.size())
		throw std::runtime_error(Logger::error("[Syntax] : 'listen' directive requires value and semicolon at line " + to_string(lineCounter_) + "."));

	std::string portStr = lineTokens[idx + 1];
	if (portStr.empty() || portStr[0] == '0')
		throw std::runtime_error(Logger::error("[Syntax] : Invalid port: 'listen' Cannot be empty or start with 0 at line " + to_string(lineCounter_) + "."));
	for (std::size_t i = 0; i < portStr.length(); ++i)
	{
		if (portStr[i]< '0' || portStr[i] > '9')
			throw std::runtime_error(Logger::error("[Syntax] : Invalid port: 'listen' Must contain only digits at line " + to_string(lineCounter_) + "."));
	}
	std::size_t port = std::atoll(portStr.c_str());
	if (portStr.size() >= 6 || port < 1 || port > 65535)
		throw std::runtime_error(Logger::error("[Syntax] : Invalid port: 'listen' Must be in range 1-65535 at line " + to_string(lineCounter_) + "."));
	if (lineTokens[idx + 2] != ";")
		throw std::runtime_error(Logger::error("[Syntax] : 'listen' Missing ';' after port number at line " + to_string(lineCounter_) + "."));
	ServerConfig& current_server = servers_.back();
	if (current_server.Tracker.has_port == true)
		throw std::runtime_error(Logger::error("[Syntax] : Multiple 'listen' directives detected in configuration at line " + to_string(lineCounter_) + "."));
...
{
	if (idx + 2 >= lineTokens.size())
		throw std::runtime_error(Logger::error("[Syntax] : 'host' directive requires value and semicolon at line " + to_string(lineCounter_) + "."));
	if (lineTokens[idx + 2] != ";")
		throw std::runtime_error(Logger::error("[Syntax] : 'host' Missing ';' after Host type at line ") + to_string(lineCounter_) + ".");	

	ServerConfig& current_server = servers_.back();
	if (current_server.Tracker.has_host == true)
		throw std::runtime_error(Logger::error("[Syntax] : Multiple 'host' directives detected in configuration at line " + to_string(lineCounter_) + "."));
...
void ServerConfigParser::parseServerName(std::vector<std::string > &lineTokens, std::size_t &idx)
{
	...
	if (idx + 2 >= lineTokens.size())
		throw std::runtime_error(Logger::error("[Syntax] : 'server_name' needs value and semicolon at line " + to_string(lineCounter_) + "."));
	...
	while (idx < lineTokens.size() && lineTokens[idx] != ";")
	{
		...
		if (host == "{" || host == "}")
			throw std::runtime_error(Logger::error("[Syntax] : 'server_name' Unexpected '{' or '}' in server_name at line " + to_string(lineCounter_) + "."));
		bool hasColon = false;
		...
		for (std::size_t i = 0; i < host.length(); ++i)
		{
			...
			if (c == ':')
			{
				if (i == 0 || i == host.length() - 1)
					throw std::runtime_error(Logger::error("[Syntax] : ':' cannot be at start or end of server_name at line " + to_string(lineCounter_) + "."));
				if (hasColon)
					throw std::runtime_error(Logger::error("[Syntax] : Multiple ':' in server_name at line " + to_string(lineCounter_) + "."));
				hasColon = true;
				numberAllowed = true;
				continue;
			}
			...
			{
				if (!numberAllowed)
					throw std::runtime_error(Logger::error("[Syntax] : Numbers only allowed after ':' in server_name at line " + to_string(lineCounter_) + "."));
			...
			throw std::runtime_error(Logger::error("[Syntax] : Invalid character in server_name at line " + to_string(lineCounter_) + "."));
		}

		if (hasColon && !hasNumberAfterColon)
			throw std::runtime_error(Logger::error("[Syntax] : Number required after ':' in server_name at line " + to_string(lineCounter_) + "."));

		server_names.push_back(host);
		idx++;
	}
	if (idx >= lineTokens.size() || lineTokens[idx] != ";")
		throw std::runtime_error(Logger::error("[Syntax] : Missing ';' in server_name at line " + to_string(lineCounter_) + "."));
	
	ServerConfig& current_server = servers_.back();
	if (!current_server.server_names.empty())
		throw std::runtime_error(Logger::error("[Syntax] : Multiple 'server_name' directives detected in configuration at line " + to_string(lineCounter_) + "."));
...
void ServerConfigParser::parseMaxBodySize(std::vector<std::string > &lineTokens, std::size_t &idx)
{
	std::size_t result ;
	if (idx + 2 >= lineTokens.size())
		throw std::runtime_error(Logger::error("[Syntax] : 'max_body_size' directive requires a value and a semicolon at line " + to_string(lineCounter_) + "."));
	idx++;
	std::string numStr = lineTokens[idx];
	if (numStr.length() > 20)
		throw std::runtime_error(Logger::error("[Syntax] : 'max_body_size' value is too large at line " + to_string(lineCounter_) + "."));
	...
	if (numStr.empty())
		throw std::runtime_error(Logger::error("[Syntax] : 'max_body_size' value cannot be empty at line " + to_string(lineCounter_) + "."));
	for (std::size_t i = 0; i < numStr.length(); ++i)
	{
		if (numStr[i]<'0' || numStr[i] > '9')
			throw std::runtime_error(Logger::error("[Syntax] : 'max_body_size' must be a valid number with an optional 'm', 'k', or 'g' suffix at line " + to_string(lineCounter_) + "."));
	}
	...
	idx++;
	if (lineTokens[idx] != ";")
		throw std::runtime_error(Logger::error("[Syntax] : Missing ';' in 'max_body_size' directive at line " + to_string(lineCounter_) + "."));
	ServerConfig &current_server = servers_.back();
	if (current_server.Tracker.has_max_body_size)
		throw std::runtime_error(Logger::error("[Syntax] : Multiple 'max_body_size' directives detected in configuration at line " + to_string(lineCounter_) + "."));
...
void ServerConfigParser::parseErrorPage(std::vector<std::string > &lineTokens, std::size_t &idx)
{
	if (idx + 3 >= lineTokens.size())
		throw std::runtime_error(Logger::error("[Syntax] : 'error_page' directive requires a status code, a file path, and a semicolon at line " + to_string(lineCounter_) + "."));
	idx++;
	std::string statusCodeStr = lineTokens[idx];
	if (statusCodeStr.empty() || statusCodeStr[0] == '0')
		throw std::runtime_error(Logger::error("[Syntax] : Invalid status code in 'error_page': Cannot be empty or start with 0 at line " + to_string(lineCounter_) + "."));

	for (std::size_t i = 0; i < statusCodeStr.length(); ++i)
	{
		if (statusCodeStr[i]<'0' || statusCodeStr[i] > '9')
			throw std::runtime_error(Logger::error("[Syntax] : Invalid status code in 'error_page': Must contain only digits at line " + to_string(lineCounter_) + "."));
	}

	std::size_t statusCode = std::atoll(statusCodeStr.c_str());
	if ((statusCodeStr.size() > 3) || statusCode < 100 || statusCode > 599)
		throw std::runtime_error(Logger::error("[Syntax] : Invalid status code in 'error_page': Must be in range 100-599 at line " + to_string(lineCounter_) + "."));

	idx++;

	std::string filePath = lineTokens[idx];
	if (filePath.empty())
		throw std::runtime_error(Logger::error("[Syntax] : Invalid file path in 'error_page': Cannot be empty at line " + to_string(lineCounter_) + "."));
	if (filePath[0] != '/')
		throw std::runtime_error(Logger::error("[Syntax] : Invalid file path in 'error_page': Must start with '/' at line " + to_string(lineCounter_) + "."));

	idx++;
	if (lineTokens[idx] != ";")
		throw std::runtime_error(Logger::error("[Syntax] : 'error_page' directive must end with a semicolon at line " + to_string(lineCounter_) + "."));
	ServerConfig& current_server = servers_.back();
	current_server.error_page[to_string(statusCode)] = 	filePath;
}
...
void ServerConfigParser::parseRoute(std::vector<std::string > &lineTokens, std::size_t &idx)
{
	...
	if (idx >= lineTokens.size())
		throw std::runtime_error(Logger::error("[Syntax] : No route specified after 'route' keyword at line " + to_string(lineCounter_) + "."));

	if (((idx + 1) < lineTokens.size() && lineTokens[idx + 1] != "{"))
		throw std::runtime_error(Logger::error("[Syntax] : Multiple routes defined at line " + to_string(lineCounter_) + "."));
	std::string route = lineTokens[idx];
	if (route.empty() || route[0] != '/')
		throw std::runtime_error(Logger::error("[Syntax] : Route must be start with '/' at line " + to_string(lineCounter_) + "."));
	...
	if (route != "default" && route != "/")
	{
		for (std::size_t i = 1; i < route.length(); i++)
		{
			if (route[i] != '/' && !std::isalpha(static_cast <unsigned char> (route[i])))
				throw std::runtime_error(Logger::error("[Syntax] : 'route' Only alphabetic characters allowed between slashes at line " + to_string(lineCounter_) + "."));
		}
	}
...
void ServerConfigParser::parseMethods(std::vector<std::string> &lineTokens, std::size_t &idx)
{
	...
	if (idx + 2 >= lineTokens.size())
		throw std::runtime_error(Logger::error("[Syntax] : 'methods' directive requires at least one value at line " + to_string(lineCounter_) + "."));
	idx++;
	while (idx < lineTokens.size() && lineTokens[idx] != ";" && lineTokens[idx] != "{")
	{
		...
		{
			if (isGet)
				throw std::runtime_error(Logger::error("[Syntax] : Duplicate method 'GET' found at line " + to_string(lineCounter_) + "."));
			isGet = true;
		...
		{
			if (isPost)
				throw std::runtime_error(Logger::error("[Syntax] : Duplicate method 'POST' found at line " + to_string(lineCounter_) + "."));
			isPost = true;
		...
		{
			if (isDelete)
				throw std::runtime_error(Logger::error("[Syntax] : Duplicate method 'DELETE' found at line " + to_string(lineCounter_) + "."));
			isDelete = true;
			Methods.push_back("DELETE");
		}
		else
			throw std::runtime_error(Logger::error("[Syntax] : Unsupported method '" + lineTokens[idx] + "' at line " + to_string(lineCounter_) + "."));
		idx++;
	}
	if (lineTokens[idx] != ";")
		throw std::runtime_error(Logger::error("[Syntax] : 'methods' directive must end with a semicolon at line " + to_string(lineCounter_) + "."));

	
	
	Route &current_route = servers_.back().routes.back();
	if (!current_route.accepted_methods.empty())
		throw std::runtime_error(Logger::error("[Syntax] : Multiple 'methods' directives detected in configuration at line " + to_string(lineCounter_) + "."));
...
{
	if (idx + 2 >= lineTokens.size())
		throw std::runtime_error(Logger::error("[Syntax] : 'root' requires a path and semicolon at line " + to_string(lineCounter_) + "."));
	idx++;
	...
	for (std::string::size_type i = 0; i < path.size(); ++i)
	{
		if (	!std::isalpha(static_cast<unsigned char> (path[i])) &&
				...
				path[i] != '/')
			throw std::runtime_error(Logger::error("[Syntax] : 'root' path contains invalid character at line " + to_string(lineCounter_) + "."));
	}

	idx++;
	if (lineTokens[idx] != ";")
		throw std::runtime_error(Logger::error("[Syntax] : 'root' directive must end with a semicolon at line " + to_string(lineCounter_) + "."));
	Route &current_route = servers_.back().routes.back();
	if (current_route.Tracker.has_root_dir == true)
		throw std::runtime_error(Logger::error("[Syntax] : Multiple 'root' directives detected in configuration at line " + to_string(lineCounter_) + "."));
...
{
	if (idx + 2 >= lineTokens.size())
		throw std::runtime_error(Logger::error("[Syntax] : 'DirectoryListing' needs value and semicolon at line " + to_string(lineCounter_) + "."));
	idx++;
	if (lineTokens[idx] != "on" && lineTokens[idx] != "off")
		throw std::runtime_error(Logger::error("[Syntax] : 'DirectoryListing' must be on|off at line " + to_string(lineCounter_) + "."));
	idx++;
	if (lineTokens[idx] != ";")
		throw std::runtime_error(Logger::error("[Syntax] : 'DirectoryListing' directive must end with a semicolon at line " + to_string(lineCounter_) + "."));
	Route &current_route = servers_.back().routes.back();
	if (current_route.Tracker.has_directory_listing == true)
		throw std::runtime_error(Logger::error("[Syntax] : Multiple 'has_directory_listing' directives detected in configuration at line " + to_string(lineCounter_) + "."));
...
{
	if (idx + 2 >= lineTokens.size())
		throw std::runtime_error(Logger::error("[Syntax] : 'default_file' needs value and semicolon at line " + to_string(lineCounter_) + "."));
	idx++;
	...
	for (std::string::size_type i = 0; i < file.size(); ++i)
	{
      ...
      if (!std::isalnum(c) && c != '/' && c != '.' && c != '-' && c != '_')
			throw std::runtime_error(Logger::error("[Syntax] : 'default_file' path contains invalid character at line " + to_string(lineCounter_) + "."));
	}
	idx++;
	if (lineTokens[idx] != ";")
		throw std::runtime_error(Logger::error("[Syntax] : 'default_file' directive must end with a semicolon at line " + to_string(lineCounter_) + "."));
	Route &current_route = servers_.back().routes.back();
	if (current_route.Tracker.has_default_file == true)
		throw std::runtime_error(Logger::error("[Syntax] : Multiple 'default_file' directives detected in configuration at line " + to_string(lineCounter_) + "."));
...
{
	if (idx + 3 >= lineTokens.size())
		throw std::runtime_error(Logger::error("[Syntax] : 'cgi' needs extension, path and semicolon at line " + to_string(lineCounter_) + "."));
	idx++;
	...
	for (std::string::size_type i = 0; i < extension.size(); ++i)
	{
		...
		if (!isValidChar(c))
			throw std::runtime_error(Logger::error("[Syntax] : 'cgi' extension contains invalid character at line " + to_string(lineCounter_) + "."));
	}
	idx++;
	...
	for (std::string::size_type i = 0; i < path.size(); ++i)
	{
		...
		if (!isValidChar(c))
			throw std::runtime_error(Logger::error("[Syntax] : 'cgi' path contains invalid character at line " + to_string(lineCounter_) + "."));
	}
	idx++;
	if (lineTokens[idx] != ";")
		throw std::runtime_error(Logger::error("[Syntax] : 'cgi' directive must end with a semicolon at line " + to_string(lineCounter_) + "."));
...
{
	if (idx + 2 >= lineTokens.size())
		throw std::runtime_error(Logger::error("[Syntax] : 'upload_directory' needs value and semicolon at line " + to_string(lineCounter_) + "."));
	idx++;
	std::string path = lineTokens[idx];
	for (std::string::size_type i = 0; i < path.size(); ++i)
	{
		unsigned char c = static_cast<unsigned char> (path[i]);
		if (!std::isalnum(c) && c != '/' && c != '.' && c != '-' && c != '_')
			throw std::runtime_error(Logger::error("[Syntax] : 'upload_directory' path contains invalid character at line " + to_string(lineCounter_) + "."));
	}
	idx++;
	if (lineTokens[idx] != ";")
		throw std::runtime_error(Logger::error("[Syntax] : 'upload_directory' directive must end with a semicolon at line " + to_string(lineCounter_) + "."));
	Route &current_route = servers_.back().routes.back();
	if (current_route.Tracker.has_upload_dir == true)
		throw std::runtime_error(Logger::error("[Syntax] : Multiple 'upload_directory' directives detected in configuration at line " + to_string(lineCounter_) + "."));
...
{
	if (idx + 2 >= lineTokens.size())
		throw std::runtime_error(Logger::error("[Syntax] : 'redirect' needs value and semicolon at line " + to_string(lineCounter_) + "."));
	idx += 2;
	if (lineTokens[idx] != ";")
		throw std::runtime_error(Logger::error("[Syntax] : 'redirect' directive must end with a semicolon at line " + to_string(lineCounter_) + "."));
	Route &current_route = servers_.back().routes.back();
	if (current_route.Tracker.has_redirect == true)
		throw std::runtime_error(Logger::error("[Syntax] : Multiple 'redirect' directives detected in configuration at line " + to_string(lineCounter_) + "."));
	...
	for (std::size_t idx = 0; idx < lineTokens.size(); ++idx)
	{
		...
		if (!insideRouteBlock_ && insideServerBlock_)
		{
			if (lineTokens[idx] == "listen")
				...
				throw std::runtime_error(Logger::error("[Syntax] : Invalid Directive in server block at line " + to_string(lineCounter_) + "."));
		}
		...
		else if (insideRouteBlock_ && insideServerBlock_)
		{
			...
			if (lineTokens[idx] == "methods")
				...
				throw std::runtime_error(Logger::error("[Syntax] : Invalid Directive in route block at line " + to_string(lineCounter_) + "."));
		}
		...
		// Error for invalid context
			...
			throw std::runtime_error(Logger::error("[Syntax] : Directive outside server block at line " + to_string(lineCounter_) + "."));
	}
}
