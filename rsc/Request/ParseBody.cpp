...
bool HttpRequest::parseChunkedBody(HttpClient &client, const Route &route)
{
	...
	std::string FileName;
	std::map<std::string, std::string>::iterator it = client.Srequest.headers.find("X-File-Name");
			...
			case STATE_FILE_CREATE:
				{
					...
					{
						std::cerr << "Error: File already exists: " << client.Srequest.filename << std::endl;
			...
			case STATE_CHUNK_SIZE:
				{
					...
					while (pos < client.bytes_received)
					{
						...
						if (c == '\r')
						{
							try
							{
								client.Srequest.chunk_size = std::strtoull(client.Srequest.chunk_size_str.c_str(), NULL, 16);
