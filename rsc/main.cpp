...
void handleNewConnection(int client_fd, int epfd, std::map<int, HttpClient> &clients)
{
	try
	{
...
void processEpollEvents(int ready_fd_count, struct epoll_event *events, std::map<int, HttpClient> &clients)
{
...
int main()
{
	try
	{
		...
		while (true)
		{
			...
			processEpollEvents(ready_fd_count, events, clients);
		}
	}
	catch (const std::exception &e)
	{
